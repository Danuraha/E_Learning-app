...
const Navbar = () => {
  ...
  const menuItems = ['Home', 'Dashboard', 'My Courses', 'About', 'Contact Us']
  ...
  return (
    ...
    >
      ...
      <div className="flex lg:flex-row md:justify-between lg:items-center text-blackOlive">
        <Link to="/">
          <div
            className="leading-loose text-3xl p-3 md:px-8 md:py-0 md:mr-10"
            style={{ fontFamily: 'Pacifico' }}
          >
            EduSpan
          </div>
        </Link>
        <div className="hidden lg:flex lg:flex-row md:space-x-10 lg:px-0 md:ml-80">
          {menuItems.map((item) => (
        ...
        <div className="hidden lg:flex md:py-3 lg:px-2">
          {token ? (
            ...
            <Link to={'/signin'}>
              <div className="bg-blue-500 hover:bg-blue-700 transform hover:skew-x-3 ... text-white font-bold py-2 px-4 rounded-lg">
                Login
              </div>
            </Link>
          )}
